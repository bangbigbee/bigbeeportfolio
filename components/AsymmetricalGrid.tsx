...
const AsymmetricalGrid: React.FC<AsymmetricalGridProps> = ({ items, reverse = false, onImageClick }) => {
        ...
        <div className="max-w-[1200px] mx-auto px-4 md:px-8">
            <div className={`grid grid-cols-1 md:grid-cols-12 gap-6 h-auto md:h-[800px]`}>
                ...
                <div className={`md:col-span-8 h-[400px] md:h-full relative overflow-hidden rounded-none group shadow-2xl ${reverse ? 'md:order-last' : ''}`}>
                    <SmartImage 
                        src={getOptimizedUrl(featured.imageUrl, 'full')} 
                        alt={featured.title} 
                        aspectRatio="fill"
                        onClick={() => onImageClick && onImageClick(featured)}
                        className="transition-transform duration-1000 group-hover:scale-110" 
                        overlay={
                            <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-12 z-20">
