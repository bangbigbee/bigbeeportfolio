...
const HorizontalSlider: React.FC<{ items: ProductCard[] }> = ({ items }) => {
    return (
                ...
                {items.map((item) => (
                    <div key={item.id} className="w-[85vw] md:w-[800px] flex-shrink-0 group">
                        <div className="relative rounded-none overflow-hidden bg-gray-100 aspect-[16/10] shadow-2xl">
                            ...
                            />
                            <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-12">
